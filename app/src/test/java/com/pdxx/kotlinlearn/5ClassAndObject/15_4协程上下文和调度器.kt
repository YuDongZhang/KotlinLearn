...
//复制代码
//当 launch {...} 在不带参数的情况下使用时，它从外部的协程作用域继承上下文和调度器。
// 在本例中，它继承于在主线程中中运行的 runBlocking 协程的上下文
...
//Dispatchers.Unconfined 调度器在调用者线程中启动一个协程，但它仅仅只是运行到第一个挂起点。在挂起之后，
// 它将恢复线程中的协程，该协程完全由调用的挂起函数决定。Unconfined 调度器适用于既不消耗CPU时间和不更新任何
// 受限于特定线程的共享数据（如UI）的协程
...
//
    ...
    //三、调试协程和线程
//协程可以在一个线程上挂起，在另一个线程上继续运行。即使使用单线程的调度器，也可能很难明确知道协程当前在做什么、
// 在哪里、处于什么状态。调试线程的常用方法是在在日志文件中为每条日志语句加上线程名，日志框架普遍支持此功能。
// 当使用协程时，线程名本身没有提供太多的上下文信息，因此 kotlinx.coroutines
// 包含了调试工具以便使协程调试起来更加容易
...
//复制代码
//log 函数在方括号中打印线程名，可以看到协程都运行于主线程，线程名后附有有当前正在执行的协程的标识符。当
// 调试模式打开时，此标识符将连续分配给所有创建的协程
...
//复制代码
//这里演示了几种新技巧。一个是对显示指定的上下文使用 runBlocking，
// 另一个是使用 withContext 函数更改协程的上下文并同时仍然保持在另一个协程中，如你在下面的输出中所看到的：
...
//
//作者：业志陈
//链接：https://juejin.cn/post/6844904100103094280
//来源：稀土掘金
