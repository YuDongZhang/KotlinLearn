...
//"本质是基于继承和实现的匿名类创建机制"
// 或者更详细地说：
// "对象表达式本质上是通过继承现有类或实现接口来创建匿名对象，允许在不声明新子类的情况下对类进行轻微改动"
...
如果你使用 匿名对象 作为 公有函数 的 返回类型 或者用作公有属性的类型，那么该 函数或属性的实际类型 
会是匿名对象声明的 超类型，
...
//    如果将匿名对象当做一个public方法返回的类型或是public属性的类型，
//    那么该方法或是属性的真正类型，就是该匿名对象所声明的父类型，如果没有声明任何父类型，那么其类型就是Any,
//    在这种情况下匿名对象中声明的任何成员都是无法访问的。
//    如下例子：
class MyClass {
    var i: Int = 0
